...
def get_project_dir(project_name: str) -> Path:
    base = Path(GENERATED_PROJECTS_DIR).resolve()
...
def write_generated_files(project_name: str, files: dict[str, str]) -> str:
    ...
    for relative_path, content in files.items():
        # Prevent path traversal: ensure resolved path stays inside project_dir
        file_path = (project_dir / relative_path).resolve()
        if not str(file_path).startswith(str(project_dir)):
            raise ValueError(
                f"Unsafe path detected and rejected: {relative_path!r}"
            )
